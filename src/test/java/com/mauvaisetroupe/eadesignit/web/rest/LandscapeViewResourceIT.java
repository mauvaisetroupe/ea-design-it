...
class LandscapeViewResourceIT {
    ...
    void getAllLandscapeViewsWithEagerRelationshipsIsNotEnabled() throws Exception {
        ...
        restLandscapeViewMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(landscapeViewRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
    ...
    @Test
    @Transactional
    void putExistingLandscapeView() throws Exception {
        // Initialize the database
        landscapeViewRepository.saveAndFlush(landscapeView);
