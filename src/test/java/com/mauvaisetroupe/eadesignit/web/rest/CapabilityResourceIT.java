...
class CapabilityResourceIT {
    ...
    void getAllCapabilitiesWithEagerRelationshipsIsEnabled() throws Exception {
        when(capabilityRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restCapabilityMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(capabilityRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllCapabilitiesWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(capabilityRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restCapabilityMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(capabilityRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
