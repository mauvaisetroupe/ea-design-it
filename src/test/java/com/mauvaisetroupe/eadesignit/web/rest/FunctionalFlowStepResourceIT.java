...
class FunctionalFlowStepResourceIT {
    ...
    void getAllFunctionalFlowStepsWithEagerRelationshipsIsEnabled() throws Exception {
        when(functionalFlowStepRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restFunctionalFlowStepMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(functionalFlowStepRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllFunctionalFlowStepsWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(functionalFlowStepRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restFunctionalFlowStepMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(functionalFlowStepRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
