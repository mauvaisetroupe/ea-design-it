...
class FunctionalFlowResourceIT {
    ...
    void getAllFunctionalFlowsWithEagerRelationshipsIsNotEnabled() throws Exception {
        ...
        restFunctionalFlowMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(functionalFlowRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
    ...
    @Test
    @Transactional
    void putExistingFunctionalFlow() throws Exception {
        // Initialize the database
        functionalFlowRepository.saveAndFlush(functionalFlow);
