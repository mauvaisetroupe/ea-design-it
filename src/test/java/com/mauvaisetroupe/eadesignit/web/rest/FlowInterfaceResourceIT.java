...
class FlowInterfaceResourceIT {
    ...
    void getAllFlowInterfacesWithEagerRelationshipsIsEnabled() throws Exception {
        when(flowInterfaceRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restFlowInterfaceMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(flowInterfaceRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllFlowInterfacesWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(flowInterfaceRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restFlowInterfaceMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(flowInterfaceRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
