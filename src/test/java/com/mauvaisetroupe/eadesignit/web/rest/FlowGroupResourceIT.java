...
class FlowGroupResourceIT {
    ...
    void getAllFlowGroupsWithEagerRelationshipsIsEnabled() throws Exception {
        when(flowGroupRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restFlowGroupMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(flowGroupRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllFlowGroupsWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(flowGroupRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restFlowGroupMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(flowGroupRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
